...
PLAYER_GROUP, ENEMY_GROUP, NEUTRAL_GROUP, INVISIBLE_GROUP = 1, 2, 3, 4
...
class ObjectType:
     ...
     def Load(self,file):
          ...
          if version == OBJTYPE_VERSION:
               ...
               self.group=load(f)
               self.colGroup=load(f)
               self.controller=load(f)
               self.sprite.Load(f)
               self.data=load(f)
          else:
               raise "Wrong ObjectType File Version!"
